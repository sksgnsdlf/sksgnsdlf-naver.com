...
export class ListEventComponent implements OnInit {
  ...
  hashTag: Array<any> = [];
  beaconEvent: any = { start_dt: '', start_time: '00:00', end_dt: '', end_time: '00:00', beacon_txt: '', get_point: 0, link_url: '' };
  beacons: Array<any> = []; // 해당 post의 비콘 리스트 
  beacons_d: Array<any> = []; // 삭제할 비콘 리스트
  beaconDevice = []; //전체 (해당 post의 society로 거름) 비콘 리스트
  applInfo: any = { state: '0', appl_txt: '', mbr_only_yn: '0', start_dt: '', end_dt: '', start_tm: '', end_tm: '', appl_cnt: 0, cls_person_txt: '', online_cnt: 0, select_typ: '1', payment_typ: '', online_yn: '0', person_restrict_cnt: 1  }; //신청 접수 설정 정보 
  ...
  getPost(no) {
    this.society.board.post.event.get(no, this.society_no)
      .subscribe((data: any) => {
  ...
  async save() {
    ...
    try {
     ...
     //post 부분 저장 
      let result: any = await this.society.board.post.event.post(formData).toPromise()
  ...
  async delete() {
    try {
      await this.society.board.post.event.delete(this.post_no, this.info.post_div).toPromise();
 ...
 searchAddressToCoordinateInput(){
  ...
  getBeacon(content) {
    ...
    //비콘 이벤트 시작 종료일시 입력 값 
    if (this.info.event_start_dttm) {
      this.beaconEvent.start_dt = this.info.event_start_dttm.split(' ')[0];
      this.beaconEvent.start_time = this.info.event_start_dttm.split(' ')[1];
      this.beaconEvent.end_dt = this.info.event_end_dttm.split(' ')[0];
      this.beaconEvent.end_time = this.info.event_end_dttm.split(' ')[1];
      this.beaconEvent.beacon_txt = this.info.event_beacon_txt;
      this.beaconEvent.get_point = this.info.event_get_point;
      this.beaconEvent.link_url = this.info.event_link_url;
    }
  ...
  saveBeaconEvent() {
    if (this.beaconEvent.start_dt || this.beaconEvent.end_dt || this.beaconEvent.beacon_txt || this.beaconEvent.link_url) {
      if (!this.beaconEvent.start_dt) {
        alert('이벤트 시작 일자를 입력해 주세요.');
        return;
      }
      if (!this.beaconEvent.end_dt) {
        alert('이벤트 마감 일자를 입력해 주세요.');
        return;
      }
      if (!this.beaconEvent.start_time) {
        alert('이벤트 시작 시간을 입력해 주세요.');
        return;
      }
      if (!this.beaconEvent.end_time) {
        alert('이벤트 마감 시간을 입력해 주세요.');
        return;
      }
      if (!isNumber(this.beaconEvent.get_point)) {
        alert('지급포인트를 입력해 주세요.');
        return;
      }
      if (!this.beaconEvent.beacon_txt) {
        alert('참여 혜택을 입력해 주세요.');
        return;
      }
      if (!this.beaconEvent.link_url) {
        alert('안내 url을 입력해 주세요.');
        return;
      }
      //비콘 이벤트 설정 
      this.info.event_start_dttm = this.beaconEvent.start_dt + ' ' + this.beaconEvent.start_time;
      this.info.event_end_dttm = this.beaconEvent.end_dt + ' ' + this.beaconEvent.end_time;
      this.info.event_beacon_txt = this.beaconEvent.beacon_txt;
      this.info.event_get_point = this.beaconEvent.get_point;
      this.info.event_link_url = this.beaconEvent.link_url;
    }
    //행사용 비콘 추가 및 삭제
