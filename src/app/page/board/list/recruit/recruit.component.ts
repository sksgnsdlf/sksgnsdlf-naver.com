...
export class ListRecruitComponent implements OnInit {
    ...
    this.society.board.post.common.get(no, this.society_no)
      ...
      .subscribe((data: any) => {
        ...
        if (this.info.district_cd)
          setTimeout(() => { this.info.district_nm = this.options.districts.find(x => x.district_cd == this.info.district_cd).district_nm; }, 500);
        //신청ㆍ접수 정보 가지고 오기 
  ...
  openImage() {
    if (this.image || this.thumbInfo.img_url) {
