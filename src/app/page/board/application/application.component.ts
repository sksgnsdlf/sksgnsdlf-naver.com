...
export class ApplicationComponent implements OnInit {
  ...
  optionSelected(society_no){
    ...
    if(this.searchForm.society_no){
      this.society.board.post.getApplOption(this.searchForm.society_no)
  ...
  searchPost(page = null){
    ...
    this.society.board.post.getApplPost(this.searchForm)
    .subscribe((data:any)=>{
    ...
    this.listTable={
      ...
      rows:this.postList.map((_,idx)=>{
        return [
            ...
            { key:'no', value:(idx+1)+((this.page-1)*10), align:'center'},
            { key:'subj', value:_.subj, align:'left'},
            { key:'board_tab_nm', value:_.board_tab_nm, align:'center'},
            { key:'user_nm', value:_.user_nm, align:'center'},
            { key:'reg_dttm', value:_.reg_dttm, align:'center'},
            { key:'state_nm', value:_.state_nm, align:'center'},
            { key:'appl_cnt', value:(_.appl_cnt?_.appl_cnt:0) + ' / ' + (_.max_appl_cnt?_.max_appl_cnt:'∞'), align:'center' }
        ]
      })
    }
  }
  ...
  getPostDetail(row){
    ...
    this.router.navigate(['board/application/detail'],{ queryParams: { post_no } });
  }
}
